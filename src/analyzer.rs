...
impl CodeAnalyzer {
	...
	fn classify_line(line: &str, lang_info: &Option<langs::Language>, comment_state: &mut CommentState) -> LineType {
		...
		if let Some(ref block_comments) = lang.block_comments {
			...
			while !line_remainder.is_empty() {
				if !comment_state.is_in_comment() {
					...
					Self::find_block_comment_end_or_nested_start(line_remainder, block_comments, nested)
				{
...
mod tests {
	...
	fn test_classify_line_blank() {
		let mut state = CommentState::new();
		assert_eq!(CodeAnalyzer::classify_line("", &None, &mut state), LineType::Blank);
		assert_eq!(CodeAnalyzer::classify_line("   ", &None, &mut state), LineType::Blank);
		assert_eq!(CodeAnalyzer::classify_line("\t\t", &None, &mut state), LineType::Blank);
		assert_eq!(CodeAnalyzer::classify_line(" \t \n ", &None, &mut state), LineType::Blank);
	}

	#[test]
	fn test_classify_line_no_language_info() {
		let mut state = CommentState::new();
		assert_eq!(CodeAnalyzer::classify_line("let x = 5;", &None, &mut state), LineType::Code);
		assert_eq!(CodeAnalyzer::classify_line("function test() {", &None, &mut state), LineType::Code);
	}
	...
	fn test_stats_collector_languages_by_lines() {
		...
		collector.add_file_stats("JavaScript".to_string(), FileStats::new(75, 60, 10, 5, 1500));
	...
	fn test_find_block_comment_start() {
		...
		assert_eq!(CodeAnalyzer::find_block_comment_start("/* comment */", &block_comments), Some((0, 2)));
		assert_eq!(CodeAnalyzer::find_block_comment_start("code /* comment", &block_comments), Some((5, 2)));
		assert_eq!(CodeAnalyzer::find_block_comment_start("no comment here", &block_comments), None);
	}

	#[test]
	fn test_find_line_comment_start() {
		let line_comments = vec!["//".to_string(), "#".to_string()];
		assert_eq!(CodeAnalyzer::find_line_comment_start("// comment", &line_comments), Some(0));
		assert_eq!(CodeAnalyzer::find_line_comment_start("code // comment", &line_comments), Some(5));
		assert_eq!(CodeAnalyzer::find_line_comment_start("# python comment", &line_comments), Some(0));
		assert_eq!(CodeAnalyzer::find_line_comment_start("code # comment", &line_comments), Some(5));
		assert_eq!(CodeAnalyzer::find_line_comment_start("no comment", &line_comments), None);
	}
